...
function genarateOrderId(){
    $.ajax({
        ...
        success(genarate){
            try {
                let lastOId =genarate.data[genarate.data.length - 1].orderID;
...
function saveConfirmOrder() {
    ...
    let o_date = $("#iDate").val();
    let o_time = $("#selectTime").val();
    let c_Id = $("#cmbCusId").val();
    let c_Name = $("#txtCusName").val();
    let s_alary = $("#txtCusSalary").val();
    let c_Address = $("#txtCusAddress").val();
    let itemCode1 = $("#cmbItemCode").val();
    ...
    for(var i of crats){
        ...
        orderDetalsDb.push(orderDetailsDTO);
